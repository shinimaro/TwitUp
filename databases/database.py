...
class Database:
    ...
    async def get_tasks_user(self, tg_id):
        async with self.pool.acquire() as connection:
            ...
            check_tasks = await connection.fetchrow("SELECT tasks_msg_id FROM tasks_messages WHERE status IN ('offer', 'offer_more', 'start_task', 'process', 'checking', 'process_subscriptions', 'process_likes', 'process_retweets', 'waiting_link', 'process_comments') AND telegram_id = $1", tg_id)
            if check_tasks:
                return True
            return False
    ...
    async def get_quantity_completed(self, tasks_msg_id):
        async with self.pool.acquire() as connection:
            result = await connection.fetchrow("SELECT COUNT(*) + (SELECT COUNT(*) FROM completed_tasks WHERE task_id = (SELECT task_id FROM tasks_messages WHERE tasks_msg_id = $1)) as complete FROM tasks_messages WHERE status IN ('start_task', 'process', 'checking', 'process_subscriptions', 'process_likes', 'process_retweets', 'waiting_link', 'process_comments') AND task_id = (SELECT task_id FROM tasks_messages WHERE tasks_msg_id = $1);", tasks_msg_id)
    ...
    async def get_info_to_user_and_tasks(self, tg_id):
        async with self.pool.acquire() as connection:
            async with connection.transaction():
                ...
                open_tasks = await connection.fetchrow("SELECT COUNT(*) as count_tasks FROM tasks_messages WHERE status IN ('offer', 'offer_more', 'start_task', 'process', 'checking', 'process_subscriptions', 'process_likes', 'process_retweets', 'waiting_link', 'process_comments') AND telegram_id = $1", tg_id)
    ...
    async def info_for_delete_messages(self, task_id) -> dict[int, InfoForDeleteTask]:
        async with self.pool.acquire() as connection:
            info = await connection.fetch("SELECT tasks_msg_id, telegram_id, message_id, status FROM tasks_messages WHERE task_id = $1 AND status IN ('offer', 'offer_more', 'start_task', 'process', 'checking', 'process_subscriptions', 'process_likes', 'process_retweets', 'waiting_link', 'process_comments')", task_id)
            info_dict = {}
            for i in info:
    ...
    async def change_priority_not_completed_task(self, tasks_msg_id):
        async with self.pool.acquire() as connection:
            ...
            tg_id = await self.get_telegram_id_from_tasks_messages(tasks_msg_id)
            change_priority = await self.get_priority_change()
            if status['status'] in ('start_task', 'process', 'process_subscriptions', 'process_likes', 'process_retweets', 'waiting_link', 'process_comments', 'checking'):
    ...
    async def get_users_after_up_level(self):
        async with self.pool.acquire() as connection:
            all_users = await connection.fetch("SELECT telegram_id, level, (SELECT COUNT(*) FROM completed_tasks WHERE telegram_id = tasks_distribution.telegram_id AND date_of_completion >= tasks_distribution.date_of_last_check) as completed_tasks, (SELECT COUNT(*) FROM tasks_messages GROUP BY telegram_id) as sent_tasks FROM tasks_distribution WHERE NOW() - date_update_level >= INTERVAL '7 days' AND level <> 'vacationers';")
            return {user['telegram_id']: {'completed_tasks': user['completed_tasks'], 'sent_tasks': user['sent_tasks'], 'level': user['level']} for user in all_users}
    ...
    # Узнать, активно ли высланное задание
    async def check_tasks_messages_on_active(self, tasks_msg_id):
        async with self.pool.acquire() as connection:
            check_tasks_messages = bool(await connection.fetchval("SELECT tasks_msg_id FROM tasks_messages WHERE tasks_msg_id = $1 AND status NOT IN ('completed', 'refuse', 'refuse_late', 'scored', 'fully_completed', 'hidden', 'deleted')", tasks_msg_id))
        if check_tasks_messages:
            task_id = await self.get_task_id_from_tasks_messages(tasks_msg_id)
            return await self.check_active_task(task_id)
        return False
    ...
    async def update_check_worksers_executions(self, tasks: list):
        async with self.pool.acquire() as connection:
            await connection.execute("UPDATE task_completion_check SET last_checking = NOW() WHERE tasks_msg_id = ANY($1)", tasks)
    ...
    async def get_last_trancaction_id(self):
        async with self.pool.acquire() as connection:
            last_id = await connection.fetchval("SELECT transaction_id FROM payments ORDER BY transaction_id DESC LIMIT 1")
