...
class Master:
    ...
    async def generate_main_driver(self):
        await self.watchman_webdriver_activate()  # Сразу создадим сторож-вебдрайвер для просмотра пепещиков
        while self.usable_drivers_queue.qsize() < config.webdrivers.num_webdrivers:
            ...
            tasks = [self.generate() for _ in range(difference)]
            await gather(*tasks)
    ...
    async def check_driver(self, page: Page) -> bool:
        try:
