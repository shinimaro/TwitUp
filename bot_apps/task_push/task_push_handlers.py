...
from parsing.main.main_function import main_parser
from parsing.main.parsing_functions.parsing_functions import parsing_comment_text
...
async def process_refuse_task(callback: CallbackQuery):
    ...
    account = await db.get_task_account(tasks_msg_id)
    # result = await main_parser(tasks_msg_id)
    result = {'comment': True}
    ...
    # Если пользователь завершил все задания
        ...
        if await db.check_completed(tasks_msg_id):
            await function_distributor_task_messages(result)


# Пользователь ввёл ссылку на комментарий + (убираем, чтобы пользователь случайно не набирал ссылку на пост для нового задания)
@router.message(lambda x: x.text and re.search('https://twitter\.com/([\w\d_]{3,})/status/\d{1,19}', x.text.lower()), ~StateFilter(FSMAddTask.add_post_link, FSMAccounts.add_first_account))
async def get_link_comment(message: Message):
    await message.delete()
    # Проверяем, какое задание ждёт ссылки на комментарий
    ...
    # Если вернулось id таска, которому была отправлена ссылка
        ...
        comment_text = await parsing_comment_text(result, message.text.lower())
        check_comment = await comment_check_itself(result, comment_text)
...
async def collect_reward_from_tasks(callback: CallbackQuery):
    ...
    if not check:
        await callback.answer(task_completion['not_collect_reward'])
