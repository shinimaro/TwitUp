...
from bot_apps.personal_task.personal_task_functions import TasksRange, find_range_value, format_date
from bot_apps.wordbank import BACK_MAIN_MENU, main_menu, personal_task, BACK, FORWARD, add_task, payment, help_center, \
    task_completion
from config import load_config
from databases.database import db, ActiveTasks, HistoryTasks

config = load_config()


async def personal_tasks_menu_keyboard(tg_id: int) -> BD:
...
async def active_tasks_menu_keyboard(tg_id: int, page: int) -> BD:
...
async def history_tasks_menu_keyboard(tg_id: int, page: int) -> BD:
...
def tasks_menu_keyboard(page: int, tasks: dict[int, ActiveTasks], pagination_word: str) -> BD:
...
def noneactive_task_keyboard() -> BD:
...
async def increased_executions_keyboard(tg_id: int, task_id: int) -> BD:
...
def add_new_executions_keyboard(executions: int) -> BD:
...
def warning_before_deletion_keyboard() -> BD:
    ...
    warning_before_deletion_kb.row(
        IB(text=help_center['buttons']['message_support_button'],
           url=f"tg://resolve?domain={random.choice(list(config.tg_bot.support_ids.values()))}"),
...
async def del_task_keyboard(tg_id: int, task_id: int) -> BD:
...
def editing_duplication_keyboard() -> BD:
...
def collect_fines_keyboard(send_id: int) -> BD:
