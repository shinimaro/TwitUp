...
from bot_apps.personal_task.adding_task.adding_task_text import define_price, round_numbers
from databases.database import db, LinkAction, CommentParameter


# Здесь валяются фанкшины, которые нужны были для нескольких модулей сразу, либо только для хендлеров
...
async def inirial_task_id(callback: CallbackQuery, state: FSMContext) -> int:
...
async def get_sum_penalty(task_id: int) -> float | int:
    ...
    return await db.check_balance_task(task_id) / 100 * 30
...
async def get_sum_refund_with_penalty(task_id):
    ...
    return emaining_balance - await get_sum_penalty(task_id)
...
async def duplicate_task_settings(task_id: int) -> dict:
    ...
    if all_task_parameters.comment_paremeters:
        ...
        data['accepted']['comment_parameters']['only_english'] = all_task_parameters.comment_paremeters.get('only_english')
    return data
