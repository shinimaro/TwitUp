...
async def define_price(list_actions: list, count=1) -> int | float:
    ...
    if 'subscriptions' in list_actions:
        total_sum += prices_dict['subscriptions']
    if 'likes' in list_actions:
        total_sum += prices_dict['likes']
    if 'retweets' in list_actions:
        total_sum += prices_dict['retweets']
    if 'comments' in list_actions:
        total_sum += prices_dict['comments']

    # Считает комиссию
    total_sum += (total_sum / 100 * await db.get_commission())
    total_sum *= count
    return round_numbers(total_sum)
...
async def no_money_text_builder(data, balance: float, balance_flag: bool = False):
    ...
    balance = int(balance) if isinstance(balance, float) and balance.is_integer() else round(balance, 2)
...
# Это планировалась как функция-предупреждение, которая будет точно говорить создателю задания, какие конкретно действия слишком часто выполняли по указанным ссылкам
# Из-за чего будет слишком долго выполняться тот объём, который нужен был, но, как я думаю, Макс не очень захочет что-то подобное вводить, да и она наврятли когда-нибудь понадобится
