...
from bot_apps.other_apps.wordbank import accounts, BACK, BACK_PERSONAL_ACCOUNT, personal_account, main_menu, \
    BACK_MAIN_MENU


# Клавиатура перед указанием первого аккаунта пользователя
def first_account_builder() -> IM:
    first_account_kb = BD()
...
def add_first_account_builder() -> IM:
...
def before_check_first_account_builder() -> IM:
...
def shadow_ban_keyboard_builder() -> IM:
...
def completion_add_first_account_builder() -> IM:
...
def completion_of_training_builder() -> IM:
...
def not_add_first_account_builder(attempt=1) -> IM:
...
def completion_of_training_with_setting_builder() -> IM:
...
def back_button_builder_for_education() -> IM:
...
def button_back_under_ruled_from_training_builder() -> IM:
...
def button_back_under_rules_from_end_training_builder(notifications=True) -> IM:
    button_back_under_ruled = BD()
    button_back_under_ruled.row(
        IB(text=BACK,
...
def how_up_to_level_builder() -> IM:
