...
async def _initial_task_text(tasks_msg_id: int) -> str:
    """Начальный текст с информацией о задании"""
...
def _account_for_task_text(account: str) -> str:
    """Текст об аккаунте для таска с линком"""
...
# Получить начальный список с заданиями
...
async def context_task_builder(tasks_msg_id: int | str, account: str, not_complete: str = None) -> str:
    text = await _initial_task_text(tasks_msg_id)
    text += _account_for_task_text(account)
    if not not_complete:
        text += '\nОго, ты всё уже сделал? Тогда жми <b>"ПРОВЕРИТЬ ЗАДАНИЕ"</b>👇'
    elif not_complete not in ('comments', 'comment'):
        dop_dict = {'subscriptions': 'ты не подписался на профиль', 'likes': 'ты не поставил лайк на пост', 'retweets': 'ты не ретвитнул пост', 'not_correct_comm': 'твой комментарий не подоходит под критерии, указанные автором задания'}
        text += f'\nКажется, <b>{dop_dict[not_complete]}</b>🥺\nСкореее исправляй это и жми <b>"ПРОВЕРИТЬ ЗАДАНИЕ"</b>👇'
    return text
...
async def control_statistic_builder(tg_id, tasks_msg_id):
    ...
    executions = await db.get_task_actual_limit(tasks_msg_id) if await db.check_active_task(await db.get_task_id_from_tasks_messages(tasks_msg_id)) else 0
    # Если ещё есть выполнения и задание не было завершено
    if executions > 0 and await db.task_again(tasks_msg_id):
...
async def issuance_of_reward(tasks_msg_id: int, accepted_support=False) -> str:
    """Итоговый текст над кнопкой сбора наград"""
    task_price: int = await db.get_task_price(await db.get_task_id_from_tasks_messages(tasks_msg_id))
    final_reward: float = await db.get_final_reward(tasks_msg_id)
    if task_price > final_reward:  # Если с юзера сняли за штраф, выдаём текст о награде со штрафом
        sum_fines: float = await db.get_sum_fines_stb(await db.get_telegram_id_from_tasks_messages(tasks_msg_id))
        return (task_completion['fines_on_reward'] if not accepted_support else task_completion['support_accept_task_and_fines']).format(round_numbers(sum_fines), round_numbers(final_reward))
    return (task_completion['completed_task'] if not accepted_support else task_completion['support_accept_task']).format(round_numbers(final_reward))
