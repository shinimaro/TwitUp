...
config = load_config()
supports_name = SupportName()
db = Database()


# Клавиатура под отправкой пользователю уведомления о задании
def new_task_keyboard_builder(tasks_msg_id) -> IM:
...
def revealing_task_builder(tasks_msg_id) -> IM:
    revealing_task = BD()
    revealing_task.row(
...
def ok_button_builder(tasks_msg_id) -> IM:
    ok_button_kb = BD()
    ok_button_kb.row(
        IB(text='Ок',
...
def ok_button_two_builder(tasks_msg_id) -> IM:
    ok_button_kb = BD()
    ok_button_kb.row(
        IB(text='Понятно',
...
async def accounts_for_task_builder(accounts_dict, tasks_msg_id, page=1) -> IM:
...
async def complete_task_builder(tg_id, tasks_msg_id) -> IM:
    complete_task = BD()
    complete_task.row(
...
async def not_parsing_builder(tasks_msg_id) -> IM:
    not_parsing = BD()
    not_parsing.row(
        IB(text=task_completion['buttons']['again_check_button'],
           callback_data=f'check_complete_task_{tasks_msg_id}'),
        IB(text=task_completion['connect_to_agent_button'],
           url=f"tg://resolve?domain={await supports_name.get_support_name()}"), width=1)
    return not_parsing.as_markup()


# Клавиатура под вводом ссылки на комментарий
async def get_link_comment_builder(tg_id, tasks_msg_id) -> IM:
    get_link_comment = BD()
...
async def comment_check_builder(tg_id, tasks_msg_id) -> IM:
    comment_check = BD()
    comment_check.row(
        IB(text=task_completion['buttons']['connect_to_agent_button'],
           url=f"tg://resolve?domain={supports_name.get_support_name()}"),
        IB(text=task_completion['buttons']['refuse_task_button'],
           callback_data=f'refuse_task_{tasks_msg_id}'), width=1)
    # Если у пользователя есть ещё аккаунты, с которых он может сделать это задание, то добавляем кнопку для смены аккаунта
    if await db.accounts_for_task_other_account(tg_id, tasks_msg_id):
...
def finally_task_builder(tasks_msg_id) -> IM:
    finally_task = BD()
    finally_task.row(
...
def clear_button_builder(tasks_msg_id) -> IM:
    clear_button = BD()
    clear_button.row(
...
async def task_again_builder(tasks_msg_id) -> IM:
    task_again = BD()
    if await db.task_again(tasks_msg_id):
        task_again.row(
...
def new_account_from_task_keyboard_builder(tasks_msg_id, account) -> IM:
...
def proposal_for_review_builder() -> IM:
...
def close_kebyoard_builder() -> IM:
    keyboard = BD()
    keyboard.row(
        IB(text=notifications['buttons']['close_button'],
           callback_data='close'))
