...
from databases.dataclasses_storage import RemainingTaskBalance
...
async def open_active_task(callback: CallbackQuery, state: FSMContext):
    task_id = await initial_task_id(callback, state)
...
async def confirmation_delete_active_task(callback: CallbackQuery, state: FSMContext):
    ...
    if task_info.status == 'waiting_start':
        ...
        if await db.check_quantity_delete_task(callback.from_user.id, task_id, 3):  # Если уже 3 и более тасков рано удалено
            ...
            await callback.message.edit_text(text=await delete_task_with_penalty(task_id, sum_refund), reply_markup=await del_task_keyboard(callback.from_user.id, task_id))
        else:
    ...
    await db.delete_task(task_id, sum_refund)
...
# Открытие какой-то страницы в истории
