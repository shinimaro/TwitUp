...
from databases.dataclasses_storage import CommentParameter, ActiveTasks, HistoryTasks, ActiveTask, \
    InfoIncreasedExecutions, HistoryTask, RemainingTaskBalance, ActionsInfo
...
async def history_tasks_menu_text(tg_id: int, page: int):
    ...
    for task_id in list(tasks.keys())[tasks_range.lower_limit:tasks_range.upper_limit]:
        ...
        text += personal_task['history_tasks_frame'].format(
            '✅' if task_info.date_of_completed else '❎',
            task_info.task_number,
            format_date(task_info.date_of_creation),
            ...
            text_for_task_actions(task_info.type_action))
    return text
...
async def active_task_text(task_id: int) -> str:
    ...
    return personal_task['active_task_info_frame'].format(
        ...
        text_for_task_actions(task.actions_info),
        text_for_user_links(task.actions_info.type_action),
        task.status.value,
...
async def insufficient_balance_for_executions(tg_id: int, task_id: int, executions: int) -> str:
    """Билдер всплывающего текста о том, что баланса уже не хватает для добавления нужного кол-ва выполнений"""
...
async def define_warning_text_before_deletion(callback: CallbackQuery, task_id: int) -> str:
    ...
    if task_info.status == 'waiting_start':  # Без предупреждения
        return _warning_delete_note_start_task(task_info)
    elif await db.check_quantity_delete_task(callback.from_user.id, task_id, 3):  # Опасное предупреждение
        return await _warning_about_rewards_distribution(task_info)
...
async def delete_task_with_penalty(task_id: int, sum_refund: float) -> str:
...
def _get_time_completed_text(task_info: HistoryTask | HistoryTasks) -> str:
...
def _get_correct_date(date: datetime.timedelta):
    ...
    if date.days > 0:
        days = str(date.days)
        date_dict = {'1': 'день', '2': 'дня', '3': 'дня', '4': 'дня', 'exceptions': ['11', '12', '13', '14']}
        text = f"{days} {'дней' if days in date_dict['exceptions'] else date_dict.get(days[-1], 'дней')}"

    if int(hours) > 0:
        hurs_dict = {'1': 'час', '2': 'часа', '3': 'часа', '4': 'часа', 'exceptions': ['11', '12', '13', '14']}
        text.append(f"{hours} {'часов' if hours in hurs_dict['exceptions'] else hurs_dict.get(hours[-1], 'часов')}")
    if int(minutes) > 0:
        minute_dict = {'1': 'минуту', '2': 'минуты', '3': 'минуты', '4': 'минуты', 'exceptions': ['11', '12', '13', '14']}
...
async def _warning_about_rewards_distribution(task_info: RemainingTaskBalance) -> str:
    ...
    award_fines = await db.get_fines_task_persent()
    send_rewards = round_numbers(task_info.balance_task / 100 * award_fines)
...
def text_for_task_actions(actions_info: ActionsInfo) -> str:
    """Билдер текста действий в задании со ссылкой"""
    actions_info = sorted_actions_dict(actions_info)
    text = ''
    ...
    if actions_info.comment_paremeters:
        text += text_for_task_comment(actions_info.comment_paremeters)
    return text


def text_for_task_comment(comment_parameter: CommentParameter) -> str:
...
def sorted_actions_dict(actions_dict: ActionsInfo) -> ActionsInfo:
    """Отсортировать словарь с типами тасков"""

    words_actions = {1: 'subscriptions', 2: 'likes', 3: 'retweets', 4: 'comments'}
    sorted_words_actions = {}
    keys = list(actions_dict.type_action.keys())
    for key, value in words_actions.items():
        if value in keys:
            sorted_words_actions[value] = actions_dict.type_action[value]
    return ActionsInfo(type_action=sorted_words_actions, comment_paremeters=actions_dict.comment_paremeters)


def text_for_user_links(type_action: ActionsInfo.type_action) -> str:
