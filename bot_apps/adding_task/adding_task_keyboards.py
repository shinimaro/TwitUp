...
from bot_apps.adding_task.adding_task_text import define_price
from bot_apps.wordbank import add_task, BACK, BACK_MAIN_MENU, payment


# Клавиатура для настройки выбора действий, которые будут в задании
async def select_actions_builder(actions=None):
...
async def setup_all_components_builder(actions=None, accepted=None):
...
async def back_button_to_setting_task_builder():
...
async def comment_parameters_builder(accepted):
...
async def comment_criteria_builder(accepted):
    comment_parameters = BD()
...
async def back_to_checking_comment_builder():
...
async def select_minimum_parameters_builder(type_setting='words'):
...
async def add_note_in_comment_builder(accepted):
    back_button = BD()
...
async def do_you_really_want_to_leave_builder():
...
async def not_payments_builder():
    not_payments = BD()
    not_payments.row(
...
async def not_money_keyboard_builder(balance_flag=False):
...
async def add_number_user_builder(balance, data):
    ...
    if result >= 6:
        add_number_user.row(
            IB(text=add_task['buttons']['choice_max_button'].format(result),
               callback_data=f'choice_execute_{result}'))
...
async def final_add_task_builder():
    final_add_task = BD()
    final_add_task.row(
...
async def add_task_keyboad_builder():
    add_task_keyboad = BD()
