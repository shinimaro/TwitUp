...
from databases.dataclasses_storage import TaskInfo
...
class SelectionWorkers:
    ...
    def __init__(self, task_id: int = 0, max_increase: int = 0, number_round: Literal[1, 2, 3] = None, precise_quantity: int = None):
        ...
        self.precise_quantity = precise_quantity
    ...
    async def selection_of_workers(self) -> dict[int, int]:
        ...
        self.selection_coefficients: SelectionCoefficients = await self._easy_selection_of_workers() \
            if self.max_increase else self._precise_selection_of_workers()
    ...
    async def selection_of_workers_for_round(self) -> dict[int, int]:
        ...
        if len(self.main_workers_dict[self.number_round]) < self.selection_coefficients.count_workers:
    ...
    # Строгий отбор конкретного числа воркеров
    def _precise_selection_of_workers(self) -> SelectionCoefficients:
        return SelectionCoefficients(count_workers=self.precise_quantity, min_count_accounts=self.precise_quantity)
    ...
    async def _complex_calculations(self) -> None:
        ...
        task_completion_rate: float = await self.calculate_executions_rate()
    ...
    async def _strict_selection_of_workers(self) -> SelectionCoefficients:
        ...
        executions = await db.get_amount_executions(self.task_id)
        count_workers = math.ceil(executions / 100 * percen_workers[self.number_round])
        # min_number_accounts = count_workers  # Хз, возможно тут нужно по-максимуму раздавать
        return SelectionCoefficients(count_workers=count_workers, min_count_accounts=count_workers)

    # Получить коэфициент выполнений
    @staticmethod
    async def calculate_executions_rate() -> float:
    ...
    def _merge_workers(self) -> dict[int, int]:
        self.finally_dict.update(self.selected_beginners)
    ...
    def _get_the_required_amount(self) -> dict[int, int]:
        ...
        counter = 0
        while workers and need_accounts > counter:  # Пока не будут взяты все доп. воркеры, либо не доберётся нужное их кол-во
    ...
    async def _selection_workers_for_last_round(self) -> int:
        ...
        completions_left = task_info.executions - total_executions
    ...
    def _distribution_workers_from_all_rounds(self) -> dict[int, int]:
        ...
        self._add_all_workers_from_round(self.number_round)
        variants = {
            1: Variant(2, 3),
            2: Variant(1, 3),
            3: Variant(2, 1)}
        ...
        if self._get_need_workers() >= len(self.main_workers_dict[variants[self.number_round].one_variant]):
            self._add_all_workers_from_round(variants[self.number_round].one_variant)
            # Если и в следующем круге не хватает людей, опять добавляем всех, что есть
            if self._get_need_workers() >= len(self.main_workers_dict[variants[self.number_round].two_variant]):
                self._add_all_workers_from_round(variants[self.number_round].two_variant)
            else:
                self._fill_out_list_workers()
        else:
            self._fill_out_list_workers()
    ...
    def _get_need_workers(self) -> int:
        return self.selection_coefficients.count_workers - len(self.selected_workers)
